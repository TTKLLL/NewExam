...
public partial class TeacherManage_AdminInfo : BasePage
{
    ...
    {
        var TeacherInfo = (from p in db.Proc_SellectAllTeacher()
                           where p.TName.Contains(Query) || p.TId.Contains(Query)
                           select p).Where(t => t.TeacherState == (int)TeacherBLL.TeacherState.OnWork)
                           .OrderByDescending(t => t.IsAdmin).ToList();
                           
                           //.Where(t => t.IsAdmin == (int)TeacherBLL.IsAdmin.Admin).ToList();
    ...
    protected void bt_Query_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("AdminInfo.aspx?Query=" + Query);
    }
    ...
    protected void rp_TeacherInfo_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            //Label isExamTeacher_lb = (Label)e.Item.FindControl("IsExamTeacher_lb");
            //CheckBox isExamTeacher = (CheckBox)e.Item.FindControl("isExamTeacher");
            //if (isExamTeacher_lb.Text == "1")
            //{
    ...
    protected void ck2_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (idLabel.Text == Request.Cookies["uId"].Value)
        {
             infoAlert("无权限修改自己");
            ck.Checked = !isCheck;
            return;
        }
        int res = teacherBll.SetAdmin(idLabel.Text.Trim());
        if (res == 1)
        {
            // SuAlert("修改成功");
            BindData();
    ...
    protected void isExamTeacher_CheckedChanged(object sender, EventArgs e)
    {
        ...
        RepeaterItem item = (RepeaterItem)ck.NamingContainer;
        Label idLabel = (Label)item.FindControl("Label3");
        if (teacherBll.SetAdmin(idLabel.Text) > 0)
        {
            SuAlert("修改成功");
            BindData();
        }
