...
public partial class InvigilateTeacher_WaitCheckStu : BasePage
{

    public List<Student> waitCheckStu; //待登记
        ...
        if (Request.QueryString["sIds"] != null)
        {
            ...
            for (int i = 0; i < sIds.Length; i++)
            {
                ...
                if (examBll.ChangeStuExamState(sIds.GetValue(i).ToString(), tId, nowExam.ExamId.ToString(),
                    (int)StudentBLL.StudentExamState.AdmitExam) <= 0)
                {
                    errAlert("学号为" + sIds.GetValue(i).ToString() + "的考生登记失败");
                    flag = 0;
                    break;
                }
            }
            if (flag == 1)
                SuAlert("成功登记 " + sIds.Length.ToString() + " 名考生");
        }
